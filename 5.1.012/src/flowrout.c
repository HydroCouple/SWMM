...
//
{
    ...
    while ( iter < MAXITER && !stopped )
    {
        ...
        if ( v2 > project->Node[i].fullVolume )
        {
            ...
            if ( project->Node[i].overflow < FUDGE ) project->Node[i].overflow = 0.0;            //(5.1.012)
...
////  Following section revised for release 5.1.012.  ////                     //(5.1.012)
    ...
    if ( project->Node[j].type == STORAGE )
    {	
        if ( project->Node[j].updated == FALSE )
            updateStorageState(project, j, project->Nobjects[LINK], NULL, dt);
        return; 
    }
//////////////////////////////////////////////////////////
