...
static int  readEvent(Project *project, char* tok[], int ntoks);                                 //(5.1.011)
...
//
{
    ...
    switch( objType )
    {
      ...
      case s_EVENT: project->NumEvents++; break;                                        //(5.1.011)
    }
    return errcode;
}
...
//
{
    ...
    switch (sect)
    {
      ...
      case s_EVENT:
        return readEvent(project, project->Tok, project->Ntokens);                                        //(5.1.011)
...
int  readEvent(Project *project, char* tok[], int ntoks)
{
    DateTime x[4];
    ...
    project->Event[project->Mevents].start = x[0] + x[1];
    project->Event[project->Mevents].end = x[2] + x[3];
    if ( project->Event[project->Mevents].start >= project->Event[project->Mevents].end )
