...
////  Following code segment added to release 5.1.011.  ////                   //(5.1.011)
        ...
        if ( project->Link[j].type != CONDUIT && k == RECT_OPEN )
        {
            x[2] = 0.0;
            x[3] = 0.0;
        }
...
//
{
    ...
    switch (type)
    {
      ...
      case WEIR:
        ...
        project->Weir[k].roadWidth    = x[7] / UCF(project, LENGTH);                             //(5.1.011)
        project->Weir[k].roadSurface  = (int)x[8];                                      //(5.1.010)
//      project->Weir[k].shape        = -(int)x[9];  //DELETED//                        //(5.1.011)
        break;
...
//
{
    ...
    switch ( project->Link[j].type )
    {
      ...
      case OUTLET:
          ...
          {
              project->Link[j].offset1 = project->Node[project->Link[j].node2].invertElev -               //(5.1.011)
                                project->Node[project->Link[j].node1].invertElev;                //(5.1.011)
...
/////////////////////////////////////////////////////////////////
            ...
            seepLossRate = project->Link[j].seepRate * xsect_getWofY(project, xsect, depth) *    //(5.1.012)
                           length;
...
/////////////////////////////////////////////
    ...
    switch (wType)
    {
      case TRANSVERSE_WEIR:
        ...
        length -= 0.1 * project->Weir[k].endCon * h;                                    //(5.1.012)
...
// (see Metcalf & Eddy, Inc., Wastewater Engineering, McGraw-Hill, 1972 p. 164).
            *q1 = project->Weir[k].cDisch1 * pow(length, 0.83) * pow(h, 1.67);
