...
//   - Text of error message saved to global variable project->ErrorMsg.
//   - Global variable project->Warnings incremented after warning message issued.
//
//   Build 5.1.012:
...
//
{
    ...
    if ( lineCount > 0 ) WRITE(project, "");                                            //(5.1.011)
}
...
{
    double x;
    double eventStepCount = (double)project->StepCount - sysStats->steadyStateCount;    //(5.1.012)

    if ( project->Nobjects[LINK] == 0 || project->StepCount == 0
    ...
    x = (1.0 - sysStats->avgTimeStep * 1000.0 / project->NewRoutingTime) * 100.0;       //(5.1.012)
    fprintf(project->Frpt.file,
    ...
    fprintf(project->Frpt.file,
        ...
        100.0 * (double)project->NonConvergeCount / eventStepCount);                    //(5.1.012)
    WRITE(project, "");
}
...
////  Following code segment added to release 5.1.011.  ////                   //(5.1.011)
    // --- save message to project->ErrorMsg if it's not for a line of input data
    if ( project->ErrorCode <= ERR_INPUT || project->ErrorCode >= ERR_FILE_NAME )
    {                                                
        sprintf(project->ErrorMsg, error_getMsg(project->ErrorCode), s);
    }
////
}
