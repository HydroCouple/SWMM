...
//
{
    ...
    oldRunoffStep = (project->NewRunoffTime - project->OldRunoffTime) / 1000.0;                  //(5.1.011)
    ...
    if ( project->Nobjects[SUBCATCH] == 0 )
    {
        ...
        project->NewRunoffTime += (double)(1000 * project->DryStep);
    ...
    project->OldRunoffTime = project->NewRunoffTime;
    project->NewRunoffTime += (double)(1000 * runoffStep);
...
////
    ...
    if ( oldRunoffStep > 0.0 ) runoff_getOutfallRunon(project, oldRunoffStep);          //(5.1.011)
...
////  Following code segment modified for release 5.1.012.  ////               //(5.1.012)
    ...
    if ( project->IsRaining || project->HasSnow || project->HasRunoff || project->HasWetLids )
    {
        timeStep = project->WetStep;
    }
////
