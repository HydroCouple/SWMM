...
//
{
    ...
    if ( project->Node[j].type != OUTFALL )
    {
        ...
        if ( project->RouteModel == DW && project->Node[j].type != STORAGE &&                    //(5.1.011)
             newDepth + project->Node[j].invertElev + FUDGE >= project->Node[j].crownElev )
        {
...
//
{
    ...
    else if ( project->Link[j].type == CONDUIT )
    {
        ...
        k = project->Link[j].subIndex;
        if ( q >= project->Link[j].qFull * (double)project->Conduit[k].barrels )                 //(5.1.012)
            project->LinkStats[j].timeFullFlow += tStep;
