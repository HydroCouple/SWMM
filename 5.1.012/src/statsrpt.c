...
#define WRITE(project, x) (report_writeLine(project,x))
...
//
{
    ...
    if ( project->FlowUnits == MGD || project->FlowUnits == CMS ) strcpy(project->FlowFmt, "%9.3f");
    else strcpy(project->FlowFmt, "%9.2f");

    // --- volume conversion factor from ft3 to Mgal or Mliters
    if (project->UnitSystem == US) project->Vcf = 7.48 / 1.0e6;
    else                  project->Vcf = 28.317 / 1.0e6;
    ...
    if ( project->Nobjects[LINK] > 0 && !project->IgnoreRouting )
    {
        ...
        if ( project->RouteModel == DW ) writeNodeSurcharge(project);                          //(5.1.011)
...
"\n  --------------------------------------------------------------------------------------------------------");
    ...
    for ( j = 0; j < project->Nobjects[SUBCATCH]; j++ )
    {
        ...
        fprintf(project->Frpt.file, " %10.2f", x/a);
        x = project->SubcatchStats[j].runoff * project->Vcf;
...
"\n  ---------------------------------------------------------------------------------");
    ...
    for ( j = 0; j < project->Nobjects[NODE]; j++ )
    {
        ...
        fprintf(project->Frpt.file, "%7.2f  %7.2f  %7.2f  %4d  %02d:%02d  %10.2f",
            ...
            days, hrs, mins, project->NodeStats[j].maxRptDepth);                        //(5.1.011)
    }
    WRITE(project, "");
}
...
"\n  -------------------------------------------------------------------------------------------------");
    ...
    for ( j = 0; j < project->Nobjects[NODE]; j++ )
    {
        ...
        fprintf(project->Frpt.file, project->FlowFmt, project->NodeStats[j].maxLatFlow * UCF(project, FLOW));
        fprintf(project->Frpt.file, project->FlowFmt, project->NodeStats[j].maxInflow * UCF(project, FLOW));
        fprintf(project->Frpt.file, "  %4d  %02d:%02d", days1, hrs1, mins1);
                fprintf(project->Frpt.file, "%12.3g", project->NodeStats[j].totLatFlow * project->Vcf);
                fprintf(project->Frpt.file, "%12.3g", project->NodeInflow[j] * project->Vcf);
        if ( fabs(project->NodeOutflow[j]) < 1.0 )
            fprintf(project->Frpt.file, "%12.3f %s",
                (project->NodeInflow[j]-project->NodeOutflow[j])*project->Vcf*1.0e6,
...
"\n  --------------------------------------------------------------------------");
        ...
        fprintf(project->Frpt.file, project->FlowFmt, project->NodeStats[j].maxOverflow * UCF(project, FLOW));
        ...
        fprintf(project->Frpt.file, "   %4d  %02d:%02d", days, hrs, mins);
                fprintf(project->Frpt.file, "%12.3f", project->NodeStats[j].volFlooded * project->Vcf);
...
"\n  --------------------------------------------------------------------------------------------------");
        ...
        for ( j = 0; j < project->Nobjects[NODE]; j++ )
        {
            ...
            fprintf(project->Frpt.file, "    %4d  %02d:%02d  ", days, hrs, mins);
            fprintf(project->Frpt.file, project->FlowFmt, project->StorageStats[k].maxFlow*UCF(project, FLOW));
        }
        WRITE(project, "");
    }
}
...
//
{
 ...
 "\n  -----------------------------------------------------------");
        ...
        for (j=0; j<project->Nobjects[NODE]; j++)
        {
            ...
            fprintf(project->Frpt.file, " ");
            fprintf(project->Frpt.file, project->FlowFmt, x);
            fprintf(project->Frpt.file, " ");
            fprintf(project->Frpt.file, project->FlowFmt, project->OutfallStats[k].maxFlow*UCF(project, FLOW));
            fprintf(project->Frpt.file, "%12.3f", project->NodeInflow[j] * project->Vcf);
 ...
 "\n  -----------------------------------------------------------"); 
        ...
        fprintf(project->Frpt.file, project->FlowFmt, flowSum);
        fprintf(project->Frpt.file, " ");
        fprintf(project->Frpt.file, project->FlowFmt, project->MaxOutfallFlow*UCF(project, FLOW));
                fprintf(project->Frpt.file, "%12.3f", volSum * project->Vcf);
...
"\n  -----------------------------------------------------------------------------");
    ...
    for ( j = 0; j < project->Nobjects[LINK]; j++ )
    {
        ...
        fprintf(project->Frpt.file, project->FlowFmt, project->LinkStats[j].maxFlow*UCF(project, FLOW));
        fprintf(project->Frpt.file, "  %4d  %02d:%02d", days, hrs, mins);
...
"\n  ---------------------------------------------------------------------------------------------------------",
    ...
    for ( j = 0; j < project->Nobjects[LINK]; j++ )
    {
        ...
        fprintf(project->Frpt.file, " %8.2f  %10d %9.2f %9.2f %9.2f %9.3f %9.2f",
            ...
            project->PumpStats[k].volume*project->Vcf, project->PumpStats[k].energy);
